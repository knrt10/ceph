...
function TEST_pg_subcommands_old_form() {
    ...
    # Save outputs from current OSD (classic)
    local outdir=${TMPDIR:-/tmp}/pg-subcommands-$$
    mkdir -p "$outdir/classic"
    _save_outputs $pgid "$outdir/classic" || return 1
    echo "Classic outputs saved to $outdir/classic"

    # Switch to crimson OSD and restart OSD
    kill_daemons $dir TERM osd.0 || return 1
    # Use crimson-osd binary
    run_osd $dir 0 --osd-objectstore=crimson || return 1
    wait_for_clean || return 1

    # Save outputs from crimson OSD
    mkdir -p "$outdir/crimson"
    _save_outputs $pgid "$outdir/crimson" || return 1
    echo "Crimson outputs saved to $outdir/crimson"

    # Compare stable fields between classic and crimson
    echo "Comparing classic vs crimson outputs..."
    _cmp_json_field "$(<$outdir/classic/query.json)" "$(<$outdir/crimson/query.json)" '.pg.pgid' || return 1
    _cmp_json_field "$(<$outdir/classic/log.json)" "$(<$outdir/crimson/log.json)" '.op_log.pg_log_t.head' || return 1
    _cmp_json_field "$(<$outdir/classic/list_unfound.json)" "$(<$outdir/crimson/list_unfound.json)" '.missing.num_missing' || return 1
    echo "Outputs match for stable fields!"
